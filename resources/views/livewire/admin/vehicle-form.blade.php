    ...
    <form wire:submit="save" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="col-span-1 md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <flux:input wire:model="daily_rate" type="number" step="0.01" label="Daily Rate ($)" placeholder="50.00" required />
            ...
            <div class="col-span-1 md:col-span-2">
                <flux:textarea wire:model="description" label="Description" placeholder="Optional details about the vehicle..." rows="3" />
            </div>
        </div>
            ...
            @if($new_images)
                <div class="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4">
                    @foreach($new_images as $photo)
                        <div class="relative rounded-lg overflow-hidden border border-zinc-200 dark:border-zinc-700 aspect-video">
                ...
                <div class="mt-6">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        @foreach($existing_images as $image)
                            <div class="relative rounded-lg overflow-hidden border {{ $featured_image_id === $image->id ? 'border-primary-500 ring-2 ring-primary-500' : 'border-zinc-200 dark:border-zinc-700' }} aspect-video group">
                                ...
                                <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                                    ...
                                    @if($featured_image_id !== $image->id)
                                        <flux:button size="sm" variant="filled" wire:click="setFeaturedImage({{ $image->id }})" title="Set as Featured">
                                            Feature
                                        </flux:button>
                                    @endif
                                    
                                    <!-- Button to delete this image -->
                                    <flux:button size="sm" variant="danger" icon="trash" wire:click="deleteImage({{ $image->id }})" wire:confirm="Delete this image?" title="Delete Image" />
                                </div>

                                <!-- Badge displaying if it is currently featured -->
                                @if($featured_image_id === $image->id)
                                    <div class="absolute top-2 left-2 bg-primary-500 text-white text-xs font-bold px-2 py-1 rounded">
                                        Featured
                                    </div>
                                @endif
                            </div>
